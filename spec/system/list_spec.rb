...
describe '投稿のテスト' do
  ...
  describe '投稿画面のテスト' do
    ...
    context '表示の確認' do
      it 'new_list_pathが"/lists/new"であるか' do
        expect(curren_path).to eq("/lists/new")
      end
    ...
    context '投稿処理のテスト' do
      it '投稿後のリダイレクト先は正しいか' do
        fill_in 'list[title]', with: Feker::Lorem.charactor(number:10)
        fill_in 'list[body]', with: Feker::Lorem.charactor(number:30)
        click_button '投稿'
  ...
  describe '編集画面のテスト' do
    ...
    context '更新処理に関するテスト'do
      fill_in 'list[title]', with: Feker::Lorem.charactor(number:5)
      fill_in 'list[body]', with: Feker::Lorem.charactor(number:30)
      click_button '保存'
